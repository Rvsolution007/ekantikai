...
@section('title', 'Edit Field: ' . $field->display_name)
        ...
        <div class="glass rounded-2xl p-6 mb-6">
            <div class="flex items-center justify-between flex-wrap gap-4">
                <div class="flex items-center gap-4">
                    ...
                    <div>
                        <h1 class="text-2xl font-bold text-white">Edit Field</h1>
                        <p class="text-gray-400">Modify questionnaire field settings</p>
                    </div>
                </div>
                <span
                    class="px-4 py-2 rounded-xl bg-primary-500/20 text-primary-300 font-mono">{{ $field->field_name }}</span>
            </div>
        </div>

        <form action="{{ route('admin.questionnaire.fields.update', $field) }}" method="POST">
            @csrf
            @method('PUT')
                            ...
                            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                                ...
                                <div>
                                    ...
                                    <select name="field_type"
                                        ...
                                        <option value="select" {{ old('field_type', $field->field_type) == 'select' ? 'selected' : '' }}>Select</option>
                                ...
                                <div>
                                    ...
                                    <select name="options_source"
                                        class="w-full bg-dark-300 border border-white/10 rounded-xl px-4 py-3 text-white focus:border-primary-500 focus:ring-1 focus:ring-primary-500 transition-colors">
                                        <option value="none" {{ old('options_source', $field->options_source) == 'none' ? 'selected' : '' }}>None</option>
                                        <option value="catalogue" {{ old('options_source', $field->options_source) == 'catalogue' ? 'selected' : '' }}>From Catalogue
                                        </option>
                                        <option value="manual" {{ old('options_source', $field->options_source) == 'manual' ? 'selected' : '' }}>Manual Entry</option>
                                    </select>
                                </div>

                                <div class="md:col-span-2">
                                    <label class="block text-sm font-medium text-gray-300 mb-2">Catalogue Column</label>
                                    <input type="text" name="catalogue_column"
                                        value="{{ old('catalogue_column', $field->catalogue_column) }}"
                                        class="w-full bg-dark-300 border border-white/10 rounded-xl px-4 py-3 text-white placeholder-gray-500 focus:border-primary-500 focus:ring-1 focus:ring-primary-500 transition-colors"
                    ...
                    <div class="glass rounded-2xl overflow-hidden">
                        ...
                        <div class="p-6 space-y-4">
                            <label class="flex items-center gap-3 cursor-pointer group">
                                <input type="checkbox" name="is_required" {{ old('is_required', $field->is_required) ? 'checked' : '' }}
                            ...
                            <label class="flex items-center gap-3 cursor-pointer group">
                                <input type="checkbox" name="is_unique_key" {{ old('is_unique_key', $field->is_unique_key) ? 'checked' : '' }}
                                    class="w-5 h-5 rounded bg-dark-300 border-white/20 text-primary-500 focus:ring-primary-500 focus:ring-offset-0">
                            ...
                            <label class="flex items-center gap-3 cursor-pointer group">
                                <input type="checkbox" name="is_active" {{ old('is_active', $field->is_active) ? 'checked' : '' }}
                                    class="w-5 h-5 rounded bg-dark-300 border-white/20 text-primary-500 focus:ring-primary-500 focus:ring-offset-0">
                            ...
                            <div>
                                ...
                                <textarea name="ai_extraction_hints" rows="3"
                                    class="w-full bg-dark-300 border border-white/10 rounded-xl px-4 py-3 text-white placeholder-gray-500 focus:border-primary-500 focus:ring-1 focus:ring-primary-500 transition-colors resize-none">{{ old('ai_extraction_hints', $field->ai_extraction_hints) }}</textarea>
                            </div>

                            <hr class="border-white/10">

                            <div class="text-sm text-gray-400 space-y-1">
                                <p><span class="text-gray-500">Order:</span> {{ $field->sort_order }}</p>
                                <p><span class="text-gray-500">Created:</span> {{ $field->created_at->format('M d, Y') }}
                                </p>
                                <p><span class="text-gray-500">Updated:</span> {{ $field->updated_at->diffForHumans() }}</p>
                            </div>
                        </div>
                    </div>
