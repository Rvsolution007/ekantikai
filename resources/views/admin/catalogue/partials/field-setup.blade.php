...
<div class="space-y-6">
    ...
    <div class="flex items-center justify-between">
        ...
        <div class="flex items-center space-x-3">
            <!-- Sync from Workflow Button -->
            <form action="{{ route('admin.catalogue-fields.sync') }}" method="POST" class="inline">
                @csrf
                <button type="submit" class="px-6 py-3 rounded-xl bg-green-500/20 text-green-400 font-medium flex items-center space-x-2 hover:bg-green-500/30 transition-colors" onclick="return confirm('This will import fields from your Workflow (Product Questions). Existing fields will be skipped. Continue?')">
                    <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M4 4v5h.582m15.356 2A8.001 8.001 0 004.582 9m0 0H9m11 11v-5h-.581m0 0a8.003 8.003 0 01-15.357-2m15.357 2H15" />
                    </svg>
                    <span>Sync from Workflow</span>
                </button>
            </form>
            <!-- Add Field Button -->
