...
<div class="space-y-6">
    @if($fields->isEmpty())
        ...
        <div class="glass rounded-2xl p-12 text-center">
            ...
            <a href="{{ route('admin.catalogue.index', ['tab' => 'fields']) }}" class="btn-primary inline-block px-6 py-3 rounded-xl text-white font-medium">
                Go to Field Setup
            </a>
        </div>
    @else
        ...
        <div class="flex flex-wrap items-center justify-between gap-4">
            ...
            <div class="flex flex-wrap items-center gap-3">
                <button @click="$dispatch('open-import')" class="px-4 py-2 rounded-xl bg-green-600 text-white font-medium hover:bg-green-700 transition-colors flex items-center space-x-2">
                    <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M4 16v1a3 3 0 003 3h10a3 3 0 003-3v-1m-4-8l-4-4m0 0L8 8m4-4v12" />
                    </svg>
                    <span>Import Excel</span>
                </button>
                <button @click="$dispatch('open-add-product')" class="btn-primary px-4 py-2 rounded-xl text-white font-medium flex items-center space-x-2">
                    <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                ...
                @if($products->total() > 0)
                    <form action="{{ route('admin.catalogue.clear-all') }}" method="POST" class="inline" onsubmit="return confirm('Delete ALL products? This cannot be undone!')">
                        @csrf
                        @method('DELETE')
                        <button type="submit" class="px-4 py-2 rounded-xl bg-red-600/20 text-red-400 font-medium hover:bg-red-600/30 transition-colors flex items-center space-x-2">
                            <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
        ...
        @if($products->isEmpty())
            ...
            <div class="glass rounded-2xl p-12 text-center">
                ...
                <p class="text-gray-400 mb-6">Add products manually or import from Excel file.</p>
                <div class="flex items-center justify-center gap-4">
                    <button @click="$dispatch('open-add-product')" class="btn-primary px-6 py-3 rounded-xl text-white font-medium">
                        Add Product
                    </button>
                    <button @click="$dispatch('open-import')" class="px-6 py-3 rounded-xl bg-green-600 text-white font-medium hover:bg-green-700 transition-colors">
                        Import Excel
                    </button>
                </div>
            </div>
                                    ...
                                    <td class="px-6 py-4 whitespace-nowrap text-right">
                                        <div class="flex items-center justify-end space-x-2">
                                            <form action="{{ route('admin.catalogue.toggle-status', $product) }}" method="POST" class="inline">
                                                @csrf
                                                <button type="submit" class="p-2 rounded-lg {{ $product->is_active ? 'bg-yellow-500/20 text-yellow-400 hover:bg-yellow-500/30' : 'bg-green-500/20 text-green-400 hover:bg-green-500/30' }} transition-colors" title="{{ $product->is_active ? 'Deactivate' : 'Activate' }}">
